...
public class ExcelReader {
	...
	public void readData() {
		...
		try {
			...
			file = WorkbookFactory.create(new FileInputStream(System.getProperty("ResSuite") + "/ScenarioSheet.xlsx"));			
				...
				case "TEST":
					...
					if (row.getCell(runindex).getStringCellValue().equalsIgnoreCase("YES")) {
						...
						if (browCount > 0) {
							...
							//System.out.println("inside the browser count");					}
			...
			try (FileWriter json = new FileWriter(filepath)) {
				...
				//System.out.println(arrayDataComb.toString());
				json.flush();
				json.close();
			}
	...
	public static JSONArray getBrowCombination(String suiteName, String className, String methodName,
		...
		try {
			...
			JSONArray method;
			// System.out.println(suiteName + className + testXMLName +
			// methodName);
			if (suiteName.equals("Browser"))
